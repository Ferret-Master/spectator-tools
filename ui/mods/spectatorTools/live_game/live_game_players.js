...
$(document).ready(function () {
    ...
    function PlayerListViewModel() {
        ...
        self.playerWasAlwaysSpectating = ko.computed(function() { return !!self.state().playerWasAlwaysSpectating; });
        ...
        self.showAllData = ko.computed(function ()
        {
            return self.spectatorPanelMode() === 'all';
        });
        ...
        self.isPlayerKnown = function(armyId) {
            if (self.playerWasAlwaysSpectating() && !self.armyId() && self.spectatorArmyData().length > 0)
                return true;
        ...
        self.trackCommander = function() {
            api.Panel.message(api.Panel.parentId, 'track_commander', { holodeck: '', armyId: null });
        }
        ...
        self.lookAtPlayerIfKnown = function(armyId, showInPIP) {

            if (self.playerWasAlwaysSpectating() && !self.armyId() && self.spectatorArmyData().length > 0){
                api.Panel.message(api.Panel.parentId, 'track_commander', { holodeck: showInPIP ? 'pips[0]' : '', armyId: armyId });
                return;
            }

            var target = self.playerContactMap()[armyId];
            if (!target)
                return;
        ...
        self.lookAtPlayerIfKnownInPIP = function(armyId) {
            ...
            self.lookAtPlayerIfKnown(armyId, true);
        }
    ...
    {
        if (payload && payload.landing && payload.gameOptions && payload.gameOptions.isLadder1v1)
            if (payload.spectator)
                model.pinSpectatorPanel(true);
            else
                model.pinPlayerListPanel(true);
